...
| `clear_after`         | clear the input after `<clear_after>` seconds of inactivity, it would not be cleared if `<clear_after>` is below or equal to zero                                                                                                                                                                                 |
| `emit_events`         | enable/disable `User` events (useful for custom statusline component)                                                                                                                                                                                                                                             |
| `disable`             | temporarily disable screenkey for specific **filetype** (e.g. `toml`, see `:h 'filetype'`), **buftype** (e.g. `terminal`, see `:h 'buftype'`) or **mode** (e.g. `t`, see `:h vim-modes` and `:h mode()`)                                                                                                          |
...
| `keys`                | how to display the special keys (or any other keys that you want to display differently), can also be a function that returns string so you can dynamically change the visual representation of the keys based on your preferences and use cases                                                                  |
